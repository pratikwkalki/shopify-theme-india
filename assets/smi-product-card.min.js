var SMI_ProductCard = function() {
    ...
    class SMI_ProductCard {
        ...
        toolTip(sectionEl){
            const items = document.querySelectorAll(sectionEl+' .smi-variants__item');
            let isShow = false;
            
            items.forEach(item => {
                item.addEventListener('mouseover', e => {
                    const tooltip = item.querySelector('.smi-tooltip');
                    const textContentTooltip = tooltip.textContent;
                    const smiToolTipWraper = this.createElementWithClasses('div', 'smi-tooltip-wrapper');
                    const smiToolTip = this.createElementWithClasses('div', 'smi-tooltip');
                    smiToolTip.textContent = textContentTooltip;
                    smiToolTipWraper.appendChild(smiToolTip);
                    ...
                    const container = item.closest('.smi-product-card'); 
                    const containerRect = container.getBoundingClientRect();
    
    
    
                    const tooltipLeft = itemRect.left + (itemRect.width / 2) - (tooltipRect.width / 2);
                    const tooltipTop = itemRect.top - tooltipRect.height - 35;
                    
                    smiToolTipWraper.style.left = `${tooltipLeft}px`;
                    smiToolTipWraper.style.top = `${tooltipTop}px`;
                    smiToolTipWraper.style.display = 'block';
                    isShow = true;


                    document.body.appendChild(smiToolTipWraper);
    
                });
    
                item.addEventListener('mouseout', e => {
                    const tooltipAll = document.querySelectorAll('.smi-tooltip-wrapper');
