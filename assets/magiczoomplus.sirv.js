...
mzOptions.onExpandClose = function(zoom_id) {
  if(document.querySelector('.product-custom.bridal-pdp') != null && window.screen.width > 749 ) {
     MagicZoom.refresh(zoom_id);
  }
}
...
mzOptions.onExpandOpen = function() {
  ...
  let activeThumbSlide = document.querySelector('.mz-expand .magic-thumb.magic-thumb-selected img');
    console.log('onExpandOpen', arguments, activeThumbSlide);
            if(activeThumbSlide) {
...
function videoSlideUpdate() {
  ...
  if(document.querySelector('.product-custom.bridal-pdp') == null || window.screen.width < 750 ) {
    let activeThumbSlide = document.querySelector('.MagicToolboxContainer .MagicToolboxSelectorsContainer a.mz-thumb-selected');
    let mainElementContainer = document.querySelector('.MagicToolboxContainer .MagicToolboxSlides .MagicToolboxSlide');
    if(activeThumbSlide.classList.contains('video-slide')) {
       let videoUrl = activeThumbSlide.getAttribute('data-video-url');
         mainElementContainer.classList.add('hide-image');
       if(mainElementContainer.querySelector('iframe') == null) {
           console.log('add video');
          let iframeHtml = `<div class="custom-video-player"><iframe width="640" height="360" src="${videoUrl}" webkitallowfullscreen="" mozallowfullscreen="" allowfullscreen="" referrerpolicy="origin" allow="autoplay"></iframe></div>`;
          mainElementContainer.querySelector('figure.mz-figure').insertAdjacentHTML('beforeend', iframeHtml);
       }
   ...
   //============ For popup gallary swipe
   let activeThumbSlideGallery = document.querySelector('.mz-expand .magic-thumb.magic-thumb-selected img');
   if(activeThumbSlideGallery) {
              let imageAlt = activeThumbSlideGallery.getAttribute('alt');
              if(imageAlt && imageAlt.includes('custom video')) {
                    let videoUrl = '';
                    let customVideoSlide = document.querySelector('[js-custom-video-slide]');
                    if(customVideoSlide) {
...
function mtHighlightActiveSelector() {
   ...
   //console.log('ON SWIPE', el, jQuery('.MagicToolboxContainer .MagicToolboxSlide[data-slide-id="' + el.attr('data-slide-id') + '"]'));
